...
- восстановление пароля через одноразовую ссылку Supabase;
- отдельный защищённый раздел управления аккаунтом с данными учётной записи, выходом и удалением;
- активная тренировочная программа с фазами, неделями, тренировками и упражнениями;
- экран «Моя программа» и отдельный экран сегодняшней тренировки;
- ручная запись фактических повторений, рабочих весов, RPE и комментария;
- атомарное завершение тренировки и история выполненных тренировок;
- явный следующий шаг на Dashboard и перенаправление авторизованного пользователя с лендинга;
- редактирование названия и длительности активной программы;
- редактирование упражнений, подходов, повторений, рабочего веса, целевого RPE и комментариев до выполнения тренировки;
- неизменяемая история после завершения тренировки;
- форма «Сообщить о доработке» с сохранением пользователя и страницы;
- свободное описание целей и ограничений без обязательного выбора внутреннего типа;
- возможность добавлять несколько отдельных ограничений;
- unit-тесты и pgTAP-тесты схемы/RLS.

Подготовленная программа Stage 2.2A добавляется пользователем явно и не генерируется AI. Новые недели не заполняются автоматически. Внутренние классификаторы целей и ограничений сохранены только для совместимости данных и не определяют формулировку пользователя. Пока не реализованы GPT Program Builder, OpenAI API, Training Engine, wearable-интеграции, логика разминки и расширенная аналитика.
...
Локальная конфигурация отключает обязательное подтверждение email. В hosted Supabase пользователь сначала подтверждает адрес по ссылке из письма. Письма локального Supabase не отправляются наружу: их можно открыть в Mailpit по адресу [http://127.0.0.1:54324](http://127.0.0.1:54324).
...
### 4. Настроить письмо восстановления

В Supabase откройте **Authentication → Email Templates → Reset password** и используйте русскоязычный шаблон из `supabase/templates/recovery.html`. Ключевая ссылка должна передавать одноразовый token hash только в фиксированный callback приложения:
...
Приложение само передаёт в Supabase точный `redirectTo`: `<NEXT_PUBLIC_SITE_URL>/auth/recovery`. Этот URL должен присутствовать в Redirect URLs. Не добавляйте в callback пользовательский параметр внешнего перенаправления.

### 5. Проверить опубликованное приложение
...
7. На странице входа запросите восстановление пароля, откройте письмо, установите новый пароль и убедитесь, что старый пароль больше не подходит.
8. В разделе «Моя программа» добавьте подготовленную программу.
9. Откройте сегодняшнюю тренировку, заполните повторения, вес и RPE, добавьте комментарий и завершите тренировку.
10. Убедитесь, что результат появился в истории, а повторно завершить ту же тренировку нельзя.
...
│   │   ├── auth/             # регистрация, вход и восстановление пароля
│   │   ├── home/             # состояния публичной главной
│   │   ├── onboarding/       # профиль, цели, ограничения
│   │   └── training/         # программа, выполнение и история
│   ├── lib/
│   │   ├── auth/
...
│   ├── templates/           # локальные шаблоны Auth-писем
│   ├── config.toml
│   └── seed.sql
