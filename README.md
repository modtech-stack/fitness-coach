...
**Stage 2 — Training Workflow Foundation.**
...
Подготовленная программа Stage 2 добавляется пользователем явно и не генерируется AI. Пока не реализованы GPT Program Builder, OpenAI API, Training Engine, wearable-интеграции и расширенная аналитика.
...
`pnpm db:test` требует запущенный локальный Supabase. Тесты проверяют CRUD собственных данных, запрет изменения чужих записей, единственность активной основной цели, создание структуры программы, атомарную запись выполненной тренировки, каскадное удаление и RLS всех пользовательских таблиц.
